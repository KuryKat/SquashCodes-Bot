...
module.exports = {
  ...
  async execute (client) {
    ...
    const textString3 = `${client.user.username}`
    ...
    const textString2 = `#${client.user.discriminator}`
    ctx.font = 'bold 40px Genta'
    ctx.fillStyle = '#f2f2f2'
    ctx.fillText(textString2, 730, canvas.height / 2 + 58)
    // define the client count
    const textString4 = `client #${client.guild.clientCount}`
    ctx.font = 'bold 60px Genta'
    ctx.fillStyle = '#f2f2f2'
    ctx.fillText(textString4, 750, canvas.height / 2 + 125)
    // get the Guild Name
    const textString5 = `${client.guild.name}`
    ctx.font = 'bold 60px Genta'
    ctx.fillStyle = '#f2f2f2'
    ...
    const channel = client.guild.channels.cache.find(ch => ch.id === '842665869808631839')
    // send the welcome embed to there
    channel.send(welcomeembed)
    // client roles add on welcome every single role
    const roles = 'T 2'
    for (let i = 0; i < roles.length; i++) { client.roles.add(roles[i]) }
  }
}
