...
module.exports = {
  ...
  async execute (client, message, _args, log, { config, Ticket }) {
    ...
    async function del () {
      const txt = join(__dirname, `../../user/transcripts/text/${ticket.get('channel')}.txt`)
      const raw = join(__dirname, `../../user/transcripts/raw/${ticket.get('channel')}.log`)
      const json = join(__dirname, `../../user/transcripts/raw/entities/${ticket.get('channel')}.json`)
